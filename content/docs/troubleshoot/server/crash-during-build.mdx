...
- **For Building from Source Deployments:**
  ...
  - **Solution:** Offload the build process to an external [Build Server](/core/servers/build-servers), or use an alternative method such as [GitHub Actions](https://docs.github.com/en/actions) to handle the build externally. Alternatively, consider upgrading your server’s capacity.
