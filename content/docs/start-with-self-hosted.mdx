...
| [4. Server resources for your projects](#4-server-resources-for-your-projects) | Estimate resources based on the projects you plan to run. |
| [5. Configure SSH access and firewall](#5-configure-ssh-access-and-firewall) | Prepare SSH access and required firewall ports. |
...
<CoolCallout className="my-5" id="fresh-server-recommended" icon={StarSparkle} title="Fresh server recommended">

It’s best to use a fresh server for Coolify to avoid any conflicts with existing applications.

</CoolCallout>

<CoolCallout className="my-5" id="server-provider" icon={ServerCloud} title="Server provider">
...
<CoolTable
  columns={[
    { header: 'Base', icon: ServerCloud },
    { header: 'Distributions', icon: Layers },
    { header: 'Notes', icon: Notes2 },
  ]}
  rows={[
...
<CoolCallout id="operating-system-support" icon={Layers} title="Operating system support">
...
<CoolTable
  columns={[
    { header: 'Item', icon: Checklist2 },
    { header: 'Minimum', icon: ArrowsDown },
    { header: 'Notes', icon: Notes2 },
  ]}
  rows={[
...
<CoolCallout id="hardware-resource-planning" icon={SackDollar} title="Hardware resource planning for budget users">
...
<CoolCallout className="my-5" id="manual-custom-setup" icon={Setting2} title="Planning a custom manual setup?">
