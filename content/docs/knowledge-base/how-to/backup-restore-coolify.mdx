...
For application data, follow [Migrate an application](/applications/operations/migrate-between-instances).
...
1. **Install a Fresh Coolify Instance:**  
   Follow the [self-hosted setup guide](/start-with-self-hosted) to install Coolify on your new server.
