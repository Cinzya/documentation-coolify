...
- You must push the built image to a Docker registry. Coolify automates the process, but you must [authenticate the registry](/applications/builds/docker-registries#authenticate-a-private-registry) on the server.
