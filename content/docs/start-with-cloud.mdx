...
import { SelfHostedProjectResources } from '@/components/pages/start-with-self-hosted';
import {
  ArrowsRight,
  Cloud3,
  CreditCard,
...
Coolify Cloud is the managed way to use Coolify. The Coolify team runs the Coolify instance for you, and you bring the servers where your apps, databases, and services run.
...
| Section | Check |
| :--- | :--- |
| [1. How Cloud works](#1-how-cloud-works) | Understand which parts Cloud manages and which parts you manage. |
| [2. Your own server is required](#2-your-own-server-is-required) | Confirm you have server infrastructure for workloads. |
| [3. Server resources for your projects](#3-server-resources-for-your-projects) | Estimate resources based on the projects you plan to run. |
| [4. Server access requirements](#4-server-access-requirements) | Prepare SSH access, Docker support, and firewall rules. |
| [5. Shared responsibility model](#5-shared-responsibility-model) | Know what Cloud manages and what remains your responsibility. |
| [6. Subscription and server limits](#6-subscription-and-server-limits) | Choose the number of servers you want to connect. |
...
The main benefit is that you do not need to allocate CPU, RAM, or storage on your own servers for running Coolify itself. Your servers are used for your workloads.

<CoolCallout className="my-5" id="cloud-managed-control-plane" icon={StarSparkle} title="Managed control plane">
...
### 3. Server resources for your projects
...
<CoolTable
  noWrapColumns={[1, 2]}
  columns={[
    { header: 'Area', icon: Layers },
    { header: 'Managed by Cloud', icon: Cloud3 },
    { header: 'Managed by you', icon: User4 },
...
<CoolCallout className="my-5" id="cloud-shared-server-security" icon={ShieldSecurity2} title="You still secure your servers">
...
### 6. Subscription and server limits

Coolify Cloud is a pay-as-you-go service. Pricing starts at **$5/month** and scales with the number of servers you want to connect (**$3/month** per additional server). See the [pricing page](https://coolify.io/pricing) for the latest pricing details.
