...
import { HardDrive, Package, Server } from 'reicon-react';
...
Persistent storage protects data from normal container replacement, but it does not protect against accidental deletion, disk failure, server loss, or corrupted data. Back up important application and database data separately.
</Callout>
...
Coolify shows three choices under **Persistent Storage**. These dashboard choices map to two underlying Docker storage types: named volumes and bind mounts.
...
Use a [bind mount](/core/persistent-storage/storage-mounts/bind-mounts) when the source must be a known server directory. Use a [file mount](/core/persistent-storage/storage-mounts/file-mounts) when the container needs one dashboard-managed file rather than a directory of persistent data.
...
<CoolNextSteps
  cards={[
    ...
    },
    {
      ...
      description: 'Create and manage one file mounted at an exact path inside the container.',
      icon: Package,
    },
  ]}
/>

To connect object storage for supported backup workflows, read [S3 storage](/core/s3-storage/overview).
