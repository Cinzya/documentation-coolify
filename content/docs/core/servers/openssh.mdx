...
If you have not added a key yet, complete [Create and authorize an SSH key](/core/servers/add-server#create-and-authorize-an-ssh-key), then return to this step.
...
Finish the [Connect the server to Coolify](/core/servers/add-server#connect-the-server-to-coolify) steps and validate the server.
...
For a remote server, repeat the SSH key steps in [Add Server](/core/servers/add-server#create-and-authorize-an-ssh-key).

</Accordion>

<Accordion title="Why does validation time out?">

Confirm that the server IP address and SSH port are correct. Then confirm that the [firewall](/core/servers/firewall) allows the SSH connection from Coolify.
