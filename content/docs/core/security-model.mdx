...
On self-hosted instances, **Settings > Configuration > Advanced** also lets you disable API access or restrict it to specific IP addresses and CIDR ranges. Create and revoke tokens under **Security > API Tokens**.

Read [API authorization](/api-reference/authorization) for the available token permissions and request format.
...
Coolify's automated SSH connection must work without an interactive prompt. Follow the [OpenSSH guide](/knowledge-base/server/openssh) when configuring the server account and key.
  ...
  groups={[
    {
      ...
      icon: Key2,
      links: [
        { title: 'API authorization', href: '/api-reference/authorization' },
        { title: 'OAuth', href: '/knowledge-base/oauth' },
        { title: 'Terminal access', href: '/knowledge-base/server/terminal-access' },
      ],
    },
    {
      ...
      icon: Server,
      links: [
        { title: 'OpenSSH', href: '/knowledge-base/server/openssh' },
