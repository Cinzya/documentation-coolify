...
| :--- | :--- | :--- |
| More CPU, memory, or storage for the current workload | Resize the current server or move the resource to a larger server | [Servers overview](/core/servers/overview) |
...
This section covers infrastructure-level scaling across servers. For a provider-specific example, read [Load Balancing on Hetzner](/knowledge-base/how-to/hetzner-loadbalancing). To use multiple CPU cores for a Node.js process on one server, read [Node.js Multi-core Scaling](/knowledge-base/nodejs-multi-core-scaling).
