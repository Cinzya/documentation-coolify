...
- Add and validate every server in Coolify. Follow [Add Server](/core/servers/add-server) for each new server.
...
- Authenticate the SSH user configured in Coolify to that registry on every server that must push or pull the image. See [Docker Registry](/knowledge-base/docker/registry#docker-credentials).
