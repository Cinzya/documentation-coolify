...
A dedicated build server separates build load from the deployment server. Using a build server requires a registry image name so the deployment server can pull the completed image. Read [Build Server](/knowledge-base/server/build-server) for the server and registry requirements.
...
Configure a [health check](/knowledge-base/health-checks) when a rolling update must verify that the new container is ready. Without a configured or image-provided health check, Coolify cannot test application readiness before treating the new container as available.
