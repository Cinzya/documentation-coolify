...
A dedicated build server separates build load from the deployment server. Using a build server requires a registry image name so the deployment server can pull the completed image. Read [Build Server](/core/servers/build-servers) for the server and registry requirements.
