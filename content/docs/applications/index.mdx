...
Building Docker images can be resource-intensive. You can use a dedicated [build server](/core/infrastructure/servers/build-servers) to handle builds separately from your production server.
...
[Read the Persistent Storage overview](/core/persistent-storage/storage-mounts/overview).
